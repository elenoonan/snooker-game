...
function startGame(){
  ...
  //add individual hole to holes array
...
class Ball{
  constructor(x = 100, y = 100, col = "#b20000", points = 1, vx = 9, vy = 4, r = 7.5){
    ...
    this.points = points;
    this.r = r;
    //leaving out vx vy for now
    this.vx = Math.random() * 10 - 5;
    this.vy = Math.random() * 10 - 5;
    //this.bounce = false;
  }
  //leave out move function for now
  draw(x, y){
    if (x) {
      this.x = x;
    }
...
//change orangeBall to balls.forEach() for collision and update and move
//walls bounce
//pockets remove and ++ score
//score board html
//send scores to database on server
//receive update score to score board
//game over when total score achieved
